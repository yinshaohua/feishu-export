...
一个在 Windows PowerShell 中运行的飞书内容导出工具，复用 Playwright 浏览器登录态，将可查看的飞书文档、云盘文件夹和多维表格保存为本地 Markdown、Excel 与附件文件。它面向需要离线归档飞书内容、又不希望配置开放平台 API token 的用户。
...
- **运行环境**: 所有 Node.js 安装、构建、测试和运行命令必须从项目根目录在 Windows PowerShell 中执行。
- **依赖布局**: 依赖位于项目根目录 `node_modules/`；不得混用 PowerShell 与 WSL2 的原生包。
...
| PowerShell + 项目内 `node_modules` 作为主要 Node 环境 | 避免 Windows/WSL2 原生包和启动脚本混用 | ✓ Good |

---
*Last updated: 2026-09-07 after switching the primary runtime to Windows PowerShell*
